...
const NearbyPOIsSection: React.FC<NearbyPOIsSectionProps> = ({
  currentPOI
}) => {
  const {
    nearbyPOIs,
  ...
  const SkeletonCards = () => <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
  ...
  return <section ref={sectionRef}>
      <div className="flex items-center justify-between mb-6">
        ...
        {nearbyPOIs.length >= 3 && <button onClick={handleViewAll} className="flex items-center text-blue-600 hover:text-blue-700 font-medium text-sm transition-colors hover:underline">
            Vedi tutti
      ...
      {error && !isLoading && <div className="text-center py-8">
          <div className="max-w-md mx-auto">
            ...
            <button onClick={handleRetry} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors text-sm">
              Riprova
            </button>
          </div>
        </div>}
      ...
      {!isLoading && !error && nearbyPOIs.length > 0 && <>
          ...
          <div className="text-center">
            ...
            {nearbyPOIs.length >= 4 && <button onClick={handleViewAll} className="text-blue-600 hover:text-blue-700 text-sm font-medium hover:underline">
                Scopri altri luoghi nelle vicinanze →
              </button>}
          </div>
        </>}

      {!isLoading && !error && nearbyPOIs.length === 0 && hasBeenVisible && <div className="text-center py-8">
          <div className="max-w-md mx-auto">
            <div className="text-gray-400 mb-4">
              <MapPin className="h-8 w-8 mx-auto" />
            </div>
            ...
            <button onClick={handleViewAll} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors text-sm">
              Esplora la mappa
            </button>
          </div>
        </div>}
