...
const POICard: React.FC<POICardProps> = ({
  ...
  return (
    <TouchTarget variant="card" asChild className="block w-full">
      <Card className="w-full max-w-sm mx-auto mobile-shadow hover:shadow-lg transition-shadow duration-300 overflow-hidden group">
        {images && images.length > 0 ? (
          <div className="relative">
            <LazyImage
              src={images[0]}
              alt={name}
              className={`w-full ${isMobile ? 'h-40' : 'h-48'} object-cover rounded-t-md`}
            />
        ...
        <CardHeader className={`${getContainerPadding()} pb-2`}>
          <div className="flex justify-between items-start">
            <div className="flex-1 min-w-0 pr-2">
              <Link to={`/poi/${id}`} className="block">
                <h3 className={`font-semibold ${getMobileTypography('text-base')} text-gray-900 group-hover:text-blue-600 transition-colors truncate-mobile-2 leading-tight`}>
                  {name}
                </h3>
              </Link>
              <Badge variant="secondary" className={`mt-1 ${getMobileTypography('text-xs')}`}>
                {category}
              </Badge>
            </div>
        ...
        <CardContent className={`${getContainerPadding()} space-y-3`}>
          {description && (
            <HtmlContent 
              content={description} 
              className={`${getMobileTypography('text-sm')} text-gray-600 truncate-mobile-2`}
            />
          )}

          <div className={`flex flex-wrap gap-2 ${getMobileTypography('text-sm')}`}>
